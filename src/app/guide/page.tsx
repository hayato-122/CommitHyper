...
import { Clipboard, Download, X, Check } from "lucide-react";
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={onClose}>
      <div className="mx-4 w-full max-w-[640px] rounded-2xl border border-mist bg-white p-6 shadow-subtle" onClick={(e) => e.stopPropagation()}>
        <div className="mb-4 flex items-center justify-between">
          ...
          <button onClick={onClose} className="rounded-lg p-1 text-zinc-500 hover:bg-pearl hover:text-midnight-ink"><X className="h-4 w-4" /></button>
        </div>
        <pre className="mb-5 max-h-72 overflow-y-auto whitespace-pre-wrap rounded-xl bg-pearl p-4 text-caption leading-relaxed text-zinc-600">{promptContent}</pre>
        <button
          onClick={handleCopy}
          className={`flex w-full items-center justify-center gap-2 rounded-xl px-4 py-3 text-body-sm font-semibold transition-all ${copied ? "bg-leaf-soft text-white" : "bg-brand-teal text-white hover:brightness-110"}`}
        >
          {copied ? <><Check className="h-4 w-4" />コピーしました！</> : <><Clipboard className="h-4 w-4" />クリップボードにコピー</>}
        </button>
      </div>
    </div>
  );
    ...
    <div className="flex min-h-screen flex-col bg-pearl">
      <header className="flex h-14 shrink-0 items-center border-b border-mist bg-white px-6">
        <Link href="/dashboard" className="flex items-center gap-2 transition-opacity hover:opacity-80">
          <img src="/icon1.png" alt="" className="h-8 w-8 rounded-lg" />
      ...
      <main className="mx-auto w-full max-w-3xl flex-1 px-6 py-16">
        {/* Hero */}
        <ScrollReveal>
          <h1 className="text-[2rem] font-semibold leading-none tracking-tight text-midnight-ink">良いコミットメッセージとは</h1>
          <p className="mt-4 text-body leading-relaxed text-zinc-600">
            コミットメッセージは、未来の自分やチームメンバーが変更履歴を見返したときに、「何を」「なぜ」変更したのかを理解できるように書くことが最も重要です。このページでは、CommitHyper が推奨する良いコミットメッセージの書き方を説明します。
          </p>
        </ScrollReveal>
        ...
        <section className="mt-16">
          <ScrollReveal>
            ...
            <p className="mt-2 text-body text-zinc-500">サマリー（1行目）だけで伝えきれないときは、空行の後に本文を追加し、変更内容を箇条書きで書くとより読みやすくなります。</p>
            <div className="mt-6 rounded-3xl border border-mist bg-white p-6">
        ...
        <section className="mt-16">
          <ScrollReveal>
            ...
            <p className="mt-2 text-body text-zinc-500">type は変更の種類を表します。</p>
        ...
        <section className="mt-16">
          <ScrollReveal>
            ...
            <p className="mt-2 text-body text-zinc-500">具体的なコミットメッセージを見てみましょう。</p>
            <div className="mt-6 grid gap-6 sm:grid-cols-2">
              <div className="rounded-2xl border border-leaf-soft/30 bg-white p-6">
                ...
                <div className="space-y-4">
                  <div><code className="block rounded-xl bg-green-50 px-4 py-2.5 text-body-sm text-green-700">feat(auth): GitHubログインボタンを追加する</code><p className="mt-1 text-caption text-zinc-500">語尾に「〜する」をつけた日本語として自然な形</p></div>
                  <div><code className="block rounded-xl bg-green-50 px-4 py-2.5 text-body-sm text-green-700">fix(api): GitHubの空レスポンスを処理する</code><p className="mt-1 text-caption text-zinc-500">こちらも「〜する」形式。一貫性があれば問題なし</p></div>
                  <div><code className="block rounded-xl bg-green-50 px-4 py-2.5 text-body-sm text-green-700">refactor(db): クエリのN+1問題を解消する</code><p className="mt-1 text-caption text-zinc-500">scope に db を指定</p></div>
                </div>
              </div>
              <div className="rounded-2xl border border-red-200 bg-white p-6">
                <p className="mb-3 text-body-sm font-bold text-red-500">悪い例</p>
                <div className="space-y-4">
                  <div><code className="block rounded-xl bg-red-50 px-4 py-2.5 text-body-sm text-red-600">fix bug</code><p className="mt-1 text-caption text-zinc-500">どのバグを何のために直したか不明</p></div>
                  <div><code className="block rounded-xl bg-red-50 px-4 py-2.5 text-body-sm text-red-600">バグを修正</code><p className="mt-1 text-caption text-zinc-500">日本語のみ。何をどう修正したかわからない</p></div>
                  <div><code className="block rounded-xl bg-red-50 px-4 py-2.5 text-body-sm text-red-600">update</code><p className="mt-1 text-caption text-zinc-500">最もよくない例。何も伝わらない</p></div>
                </div>
              </div>
            </div>
          </ScrollReveal>
        ...
        <section className="mt-16">
          <ScrollReveal>
            ...
            <div className="mt-6 grid gap-4 sm:grid-cols-2">
              <button onClick={() => setShowModal(true)} className="flex items-center gap-3 rounded-2xl border-2 border-dashed border-zinc-300 bg-white p-5 text-left text-body-sm transition-all hover:border-zinc-400 hover:bg-zinc-50">
                <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-zinc-100"><Clipboard className="h-5 w-5 text-zinc-500" /></div>
                <div><p className="font-semibold text-zinc-600">プロンプトをコピー</p><p className="text-caption text-zinc-400">ChatGPT/Claudeに渡す</p></div>
              </button>
              <a href="/prompts/commithyper.md" download className="flex items-center gap-3 rounded-2xl border-2 border-dashed border-zinc-300 bg-white p-5 text-left text-body-sm transition-all hover:border-zinc-400 hover:bg-zinc-50">
                <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-zinc-100"><Download className="h-5 w-5 text-zinc-500" /></div>
                <div><p className="font-semibold text-zinc-600">ルールをダウンロード</p><p className="text-caption text-zinc-400">コーディングエージェント用</p></div>
              </a>
            </div>
            <div className="mt-5 rounded-xl bg-white border border-mist p-4">
              <p className="text-caption font-semibold text-zinc-500">💡 使い方</p>
              <p className="mt-2 text-caption leading-relaxed text-zinc-600">ダウンロードしたファイルをプロジェクトのルールディレクトリ（.cursor/rules/、.github/copilot-instructions.md など）に配置し、他のルールファイルから参照してください。</p>
              <pre className="mt-2 rounded-lg bg-pearl p-3 text-caption leading-relaxed text-zinc-600">{`# プロジェクトルール例
コミットメッセージを作成・評価する際は、
必ず commithyper.md のルールに従ってください。`}</pre>
            </div>
          </ScrollReveal>
        </section>
      </main>
