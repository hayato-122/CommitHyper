...
export default function DashboardPage() {
  ...
  function getCandidatePage() {
    const filtered = allCommits.filter((c) => c.status === "pending" && c.currentScore < 70);
  ...
  if (loading) {
    return <div className="flex min-h-screen items-center justify-center"><p className="text-sm text-zinc-500">読み込み中...</p></div>;
  }
    ...
    <div className="flex min-h-screen flex-col">
      <header className="flex h-14 items-center justify-between border-b border-zinc-200 bg-white px-6">
        <div className="flex items-center gap-4">
          ...
          <Link href="/dashboard" className="flex items-center gap-1 text-xs text-zinc-400 hover:text-zinc-600">
            <ArrowLeft className="h-3 w-3" />
            リポジトリ一覧
          </Link>
          <div className="h-4 w-px bg-zinc-200" />
          <span className="text-sm font-medium text-zinc-600">{owner}/{name}</span>
          {branches.length > 0 && (
            <>
              <div className="h-4 w-px bg-zinc-200" />
      ...
      <div className="flex flex-1">
        <main className="flex-1 px-8 py-10">
          <div className="mb-8">
            <h1 className="text-2xl font-semibold leading-tight tracking-tight text-midnight-ink">{owner}/{name}</h1>
            <p className="mt-2 text-sm text-zinc-500">コミットメッセージを分析・改善しましょう。</p>
          </div>

          <div className="mb-8 flex items-center justify-between">
            <div className="flex overflow-hidden rounded-lg border border-zinc-300">
              <button onClick={() => onTabChange("candidates")}
                className={`px-4 py-2 text-xs font-medium transition-all ${tab === "candidates" ? "bg-brand-teal text-white" : "bg-white text-zinc-600 hover:bg-zinc-50"}`}>改善候補</button>
              <button onClick={() => onTabChange("all")}
                className={`px-4 py-2 text-xs font-medium transition-all ${tab === "all" ? "bg-brand-teal text-white" : "bg-white text-zinc-600 hover:bg-zinc-50"}`}>すべてのコミット</button>
            </div>
            <div className="flex items-center gap-2">
              <div className="flex overflow-hidden rounded-lg border border-zinc-300">
                <button onClick={() => { setSortBy("score"); setPage(1); }}
                  className={`px-3 py-1.5 text-xs font-medium transition-all ${sortBy === "score" ? "bg-brand-teal text-white" : "bg-white text-zinc-600 hover:bg-zinc-50"}`}>スコア</button>
                <button onClick={() => { setSortBy("date"); setPage(1); }}
                  className={`px-3 py-1.5 text-xs font-medium transition-all ${sortBy === "date" ? "bg-brand-teal text-white" : "bg-white text-zinc-600 hover:bg-zinc-50"}`}>日付</button>
              </div>
              ...
              <button onClick={handleRefresh} disabled={refreshing}
                ...
                <RotateCw className={`h-4 w-4 ${refreshing ? "animate-spin" : ""}`} />再分析</button>
            </div>
          </div>
            ...
            <div className="space-y-4">
                    ...
                    <div className="rounded-xl border border-zinc-200 bg-white p-5">
                      <div className="flex gap-4">
                        <div className="min-w-0 flex-1">
                          ...
                          <p className="mt-1 text-sm leading-relaxed text-zinc-800">{commit.message}</p>
                          <p className="mt-2 text-xs text-zinc-500">{commit.authorName} · {new Date(commit.committedAt).toLocaleDateString("ja-JP")}</p>
                          {commit.firstIssue && <p className="mt-2 text-xs text-red-600">{commit.firstIssue}</p>}
                        </div>
                        <div className="flex flex-col items-end justify-between gap-2">
                          <span className={`rounded-full px-3 py-1 text-xs font-semibold ${commit.currentScore < 70 ? "bg-red-100 text-red-700" : "bg-emerald-100 text-emerald-700"}`}>{commit.currentScore}/100</span>
                          <button onClick={() => router.push(`/dashboard/${owner}/${name}/improve/${commit.id}`)} className="rounded-lg bg-brand-teal px-4 py-1.5 text-xs font-medium text-white hover:brightness-110">改善する</button>
                        </div>
                      </div>
                    </div>
                  </ScrollReveal>
              ...
              {totalPages > 1 && (
                <div className="mt-6 flex items-center justify-center gap-4">
                  <button onClick={() => setPage(page - 1)} disabled={page <= 1} className="rounded-lg border border-zinc-300 px-3 py-1.5 text-sm text-zinc-600 hover:bg-zinc-50 disabled:opacity-30">←</button>
                  <span className="text-sm font-medium text-zinc-600">{String(page).padStart(2, "0")} / {String(totalPages).padStart(2, "0")}</span>
                  <button onClick={() => setPage(page + 1)} disabled={page >= totalPages} className="rounded-lg border border-zinc-300 px-3 py-1.5 text-sm text-zinc-600 hover:bg-zinc-50 disabled:opacity-30">→</button>
                </div>
              )}
            </div>
          )}
        ...
        <aside className="w-72 shrink-0 border-l border-zinc-200 bg-zinc-50 p-5">
          {progress && (
            <div className="mb-6 rounded-xl border border-zinc-200 bg-white p-5">
              ...
              <div className="mt-1 h-2 overflow-hidden rounded-full bg-zinc-100"><div className="h-full rounded-full bg-brand-teal transition-all" style={{ width: `${progress.progress}%` }} /></div>
          ...
          <div className="rounded-xl border border-zinc-200 bg-white p-5">
            ...
            <div className="space-y-2">
              ...
              <div className="flex justify-between text-sm"><span className="text-zinc-500">改善</span><span className={`font-medium ${currentAvg - initialAvg >= 0 ? "text-emerald-600" : "text-zinc-500"}`}>{currentAvg - initialAvg >= 0 ? "+" : ""}{currentAvg - initialAvg}</span></div>
